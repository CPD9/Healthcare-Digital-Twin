/**
 * twin-app.jsx — Two-page React UI for TWIN
 *
 ...
 *
 * Globals from twin-three.js:
 *   window.initThreeJS(container)
 *   window._dnaHighlight(idx), window._dnaClearHighlight()
 *   window._dnaTooltipSet(payload)
 */
...
/* Animated avatar canvas (Three.js humanoid) */
function AvatarCanvas({ activeRegion, onRegionClick }) {
  ...
  useEffect(() => {
    ...
    if (!THREE) return; // Three.js not yet available via global

    // fallback: draw a pure CSS avatar — Three.js humanoid is in twin-avatar.js
    // The CSS avatar is rendered via the DOM overlay below
  }, []);

  /* Build humanoid body outline via SVG — no extra Three.js dependency needed */
  return (
    <div className="avatar-scene" ref={containerRef}>
      {/* Glowing DNA particle field (CSS animated) */}
      ...
      <svg className="avatar-svg" viewBox="0 0 200 400" xmlns="http://www.w3.org/2000/svg">
        ...
        {/* Neck */}
        <rect x="90" y="66" width="20" height="18"
          fill="none" stroke="rgba(0,229,255,0.3)" strokeWidth="1.5"/>
        {/* Torso */}
        <path d="M60 84 L50 200 L150 200 L140 84 Z"
          fill="none" stroke={activeRegion === 'heart' || activeRegion === 'liver' || activeRegion === 'lungs' ? 'rgba(0,229,255,0.7)' : 'rgba(0,229,255,0.35)'}
        ...
        {/* DNA helix lines flowing through body */}
        {[0,1,2,3,4].map(i => (
          <line key={i}
...
function AvatarHeader({ onBack, analysis }) {
  return (
    <header className="app-header avatar-header">
      ...
      <div className="header-btns">
        <button className="hbtn active" onClick={onBack}>
          ← Back to Explorer
        </button>
      </div>
    </header>
...
function AvatarPage({ onBack, sharedState }) {
  const { analysis, messages, chatLoading, suggestions, setSuggestions, handleSend, handleClearChat } = sharedState;
...
/* ═══════════════════════════════════════════════════════════════ */
function App() {
  const [page,        setPage]        = useState('explorer'); // 'explorer' | 'avatar'
